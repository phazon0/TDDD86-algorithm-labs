...
Boggle configBoard(){
    ...
    if (!yesOrNo("Do you want to set the board yourself (Y/N)? ")) {
        return Boggle();
    }else{

        do{
        cout << "Input a "<<Boggle::NUM_CUBES <<" character string of the layout for the board. E.g 'AAAAXXXXYYYYBBBB'" << endl;
        getline(cin, line);
        }while(line.size() != Boggle::NUM_CUBES || !isStringAlpha(line));
...
int main() {
    ...
    while (true) {
        Boggle boggle = configBoard();
        cout << boggle.board.toString() << endl;
        playOneGame(boggle,lexicon);
        cout << endl;
